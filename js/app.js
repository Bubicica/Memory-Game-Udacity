...
function match() {
	const allLockedCards = document.getElementsByClassName("card");
	
	// stop ongoing animations and get ready for the new one
	cardList[0].classList.remove("animated");
	cardList[0].classList.remove("flipInY");
	...
	if (cardList[0].innerHTML === cardList[1].innerHTML){
		...
		for (let i = 0; i < allLockedCards.length; i++){
				if (!allLockedCards[i].classList.contains("open")){
					allLockedCards[i].classList.remove("locked");
				}
			}
		cardList.length = 0;
	}
    ...
    if (event.target.nodeName === 'LI') {
		...
		if (event.target.classList.contains("locked") != true) {
			...
			if (cardList.length === 2) {
				...
				// lock all other cards no in use
				const lockCards = document.getElementsByClassName("card");
				for (let i = 0; i < lockCards.length; i++){
					lockCards[i].classList.add("locked");
				}
